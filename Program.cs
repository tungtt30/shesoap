...
namespace ConsoleApp1
{
    ...
    class Donut
    {
                ...
                void Fonut()
        {
          ...
          new char[1760]; while (true)
            {
                memset(b, ' ', 1760);
                memset(z, 0.0f, 7040); for (j = 0; 6.28 > j; j += 0.07) for (
               i = 0; 6.28 > i; i += 0.02)
                    {
                        double c = Math.Sin(i); double d
              = Math.Cos(j); double e = Math.Sin(A); double f = Math.Sin(j);
                        ...
                        int y = (int)(12 + 15 * D * (l * h * n + t * m)); int o = x
                         + 80 * y; int N = (int)(8 * ((f * e - c * d * g) * m - c * d * e - f *
                          g - l * d * n)); if (22 > y && y > 0 && x > 0 && 80 > x && D > z[o])
                        {
                            z[o] = D; b[o]
                           = ".,-~:;=!*#$@"[N > 0 ? N : 0];
                        }
                    }
                Console.Clear(); nl(b); Console.
                            Write(b); A += 0.04; B += 0.02;
            }
        }
        static void memset<T>
                               (T[] buf, T val, int bufsz)
        {
            if (buf == null) buf =
